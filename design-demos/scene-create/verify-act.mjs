...
async function shot(pg, name) {
  await pg.screenshot({ path: `scene-create/${name}.png`, fullPage: true });
  console.log('shot', `scene-create/${name}.png`);
}
...
for (const vp of [
  ...
  await back.click();  // reviewing -> capturing
  await pg.waitForTimeout(600);
  await back.click();  // capturing -> browsing
