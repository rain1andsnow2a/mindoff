...
import { createSkyDome, createStars, createMoon } from '../utils.js';
import { createFigure } from '../figure.js';

/** 深夜通话 · 海边：月下海面、沙滩、坐在岸边的人 */
export function create() {
  ...
  group.add(stars);
  const moon = createMoon({ size: 5, height: 32, angle: 0 });
  group.add(moon);
  ...
  [[-14, 4, 2.5], [18, 1, 3.5], [8, -2, 1.6]].forEach(([x, z, s]) => {
  ...
  function update(t) {
    ...
    foam.material.opacity = 0.1 + push * 0.22;
