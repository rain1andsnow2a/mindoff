...
**预期**（信箱为「来信 + 思绪」两层）：
- `来信`：最新一封在上方（未读为信封，点开变信纸），下面是往期来信；每晚 21:30（东八区）自动写一封晚间来信；每周日 20:00 一封周报；来信不设每日数量上限
- `思绪`：第 3 步倾倒出的待办（办）、灵感（想）、"说不上来的累"等情绪（情）都在这里，批注式单列，带保存期（默认一个月）
- 「我留下的」：来信页底部入口；4.6 珍藏的结算卡在这里
