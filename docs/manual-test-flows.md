...
**预期**：
- 先显示「正在回顾你的表达…」，然后出三块内容，**都由 LLM 根据这次对话生成**：
  - 「这一次，你说出了」+ 一句摘出的原话（`key_quote`）
  - 桌宠的一句肯定（`companion_comment`，带 🌿 小卡片）
  - 一句极短的未来暗示（`action_hint`）
- 可以选择「把这句话留下」（进「信箱 → 长久珍藏」）、「再试一次」、「直接离开」
- 文案不能声称治好了你或改变了真实记忆，也不能出现诊断/人格标签

以前这里显示的是空引号 + `……`，因为 `App.tsx` 渲染 `<SceneEnd>` 时**漏传了 `sceneId`**，
组件里的 `useEffect` 一进来就 early return，摘要请求压根没发出去，
「把这句话留下」也是同样原因没反应。现在两个都通了。

实测一例（站台送别场景）：
```
key_quote        : 把卡在喉咙里的「再见」好好说出来
companion_comment: 你认真把那句卡了很久的「再见」说出口，已经给这场告别攒足了温柔
action_hint      : 等他的消息，等糖炒栗子的约定
```
`action_hint` 里的「糖炒栗子」来自剧情里出现过的细节 —— 摘要确实读了对话，不是模板。

如果看到空引号 + `……`，那是后端兜底模板，说明 LLM 调用失败了，查
`docker compose logs --tail 50 backend | grep -i summarize`。
