...
export function create(): TheaterScene {
  ...
  // 机头灯 / 航行灯
  const navMatR = new THREE.MeshBasicMaterial({ color: 0xff4444 });
  const navLightR = new THREE.Mesh(new THREE.SphereGeometry(0.08), navMatR);
  navLightR.position.set(0, 1.4, -4.5);
  const navMatG = new THREE.MeshBasicMaterial({ color: 0x44ff66 });
  const navLightG = new THREE.Mesh(new THREE.SphereGeometry(0.08), navMatG);
  ...
  const mkSeatRow = (x: number, z: number, n: number, rotY = 0) => {
  ...
  const signGlowMat = new THREE.MeshBasicMaterial({ color: 0x22d3aa });
  const signGlow = new THREE.Mesh(new THREE.PlaneGeometry(2.9, 0.9), signGlowMat);
  // 模拟文字行
  for (let i = 0; i < 3; i++) {
  ...
  function update(t: number) {
    ...
    navMatR.color.setHex(0xff4444).multiplyScalar(blink);
    navMatG.color.setHex(0x44ff66).multiplyScalar(blink);
    // 信息牌轻微亮度呼吸
    signGlowMat.color.setHex(0x22d3aa).multiplyScalar(0.85 + Math.sin(t * 1.2) * 0.15);
  }
